...
#define DLT_RANSAC_MAX_ITER 1e+4
...
int main ( int argc, char * argv[] )
{
	...
	fin_cal >> dummy >> omega;
	fin_cal >> dummy >> princ_x1 >> dummy>> princ_y1;
	cout << omega << " " << princ_x1 << " " << princ_y1 << endl;
	...
	{
		CvMat *intrinsic = cvCloneMat(K_data);
		camera.vK.push_back(intrinsic);
	...
	while(vFrameOrder.size() != 0)
	{
		...
		for (int itemp = 0; itemp < vFrameOrder.size(); itemp++)
		{
			...
			int nVisible = VisibleIntersection23_Simple_fast(vFeature, cFrame1);
		...
		for (int iUsedFrame = 0; iUsedFrame < vUsedFrame.size()-1; iUsedFrame++)
		{
			...
			if (!VisibleIntersectionXOR3_mem_fast(vFeature, cFrame, vUsedFrame[iUsedFrame], cx1_vec, cx2_vec, visibleID1))
			{
...
int ExcludePointAtInfinity_mem_fast(CvMat *X, CvMat *P1, CvMat *P2, CvMat *K1, CvMat *K2, vector<int> &featureID, vector<int> &excludedFeatureID, vector<vector<double> > &cX)
{
	...
	for (int i = 0; i < X->rows; i++)
	{
		...
		if ((abs(inner) < cos(PI/180*5)) && (inner > 0))
		{			
