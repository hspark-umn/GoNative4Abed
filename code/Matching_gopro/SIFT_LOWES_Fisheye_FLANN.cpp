...
void Iterate_SIFT_STATIC_MP(vector<FrameCamera> &vFC, int currentFC, vector<flann::Index<flann::L2<int> > > &vIndex, vector<Feature> &feature_static);
using namespace std;
...
int main ( int argc, char * argv[] )
{
	...
	// Building Index
	vector<flann::Index<flann::L2<int> > > vIndex;
	//vIndex.resize(vCamera[0].vTakenFrame.size());
	//omp_set_num_threads(50);
	//#pragma omp parallel for 
	for (int iFrame = 0; iFrame < vCamera[0].vTakenFrame.size(); iFrame++)
	{
		cout << "Building " << iFrame << "th frame" << endl;
		flann::Matrix<int> descM1(new int[vFC[iFrame].vSift_desc.size()*128], vFC[iFrame].vSift_desc.size(), 128);

		for (int iDesc = 0; iDesc < vFC[iFrame].vSift_desc.size(); iDesc++)
		{
			for (int iDim = 0; iDim < 128; iDim++)
			{
				descM1[iDesc][iDim] = (int) vFC[iFrame].vSift_desc[iDesc].vDesc[iDim];
			}
		}
	
		flann::Index<flann::L2<int> > index1(descM1, flann::KDTreeIndexParams(4));
		index1.buildIndex();
	...
	//#pragma omp parallel private(vIndex)
	{
	//omp_set_num_threads(50);
	#pragma omp parallel for
	for (int iFC = 0; iFC < vFrameOrder.size(); iFC++)
	{
		...
		Iterate_SIFT_STATIC_MP(vFC, iFC1, vIndex, feature_static);
...
{ 
	...
	flann::Matrix<int> descM1(new int[cFC.vSift_desc.size()*128], cFC.vSift_desc.size(), 128);

	for (int iDesc = 0; iDesc < cFC.vSift_desc.size(); iDesc++)
	{
		for (int iDim = 0; iDim < 128; iDim++)
		{
			descM1[iDesc][iDim] = (int) cFC.vSift_desc[iDesc].vDesc[iDim];
		}
	}
	...
	//time (&end);
	//cout << "Feature building " << difftime (end,start) << endl;
	...
	for (int iSecondFrame = currentFC+1; iSecondFrame < vFC.size(); iSecondFrame++)
	{
		...
		flann::Matrix<int> descM2(new int[vFC[iSecondFrame].vSift_desc.size()*128], vFC[iSecondFrame].vSift_desc.size(), 128);
		for (int iDesc = 0; iDesc < vFC[iSecondFrame].vSift_desc.size(); iDesc++)
		{
			for (int iDim = 0; iDim < 128; iDim++)
			{
				descM2[iDesc][iDim] = (int) vFC[iSecondFrame].vSift_desc[iDesc].vDesc[iDim];
			}
		}
		...
		vIndex[vFC[iSecondFrame].frameIdx].knnSearch(descM1, result12, dist12, nn, flann::SearchParams(32));
		vIndex[cFC.frameIdx].knnSearch(descM2, result21, dist21, nn, flann::SearchParams(32));

		//time (&end);
		//cout << "FLANN " << difftime (end,start) << endl;
		//time (&start);
		...
		//time (&end);
		//cout << "Ratio test " << difftime (end,start) << endl;
		///time (&start);
		...
		//time (&end);
		//cout << "Get correspondence " << difftime (end,start) << endl;
		//time (&start);
		...
		//time (&end);
		//cout << "Pushback " << difftime (end,start) << endl;
		//time (&start);
...
int GetStaticCorrespondences(vector<Point> x1, vector<Point> x2, vector<bool> &vIsInlier)
{
	...
	//time (&end);
		cout << "Fundamental " << difftime (end,start) << endl;
