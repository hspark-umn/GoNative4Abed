				 ...
				 vector<double> &vx, vector<double> &vy, vector<double> &vz, double lambda)
{
	...
	double *work = 0;// (double*)malloc((LM_DIF_WORKSZ(poi_trajectory.size(), measurement.size())+poi_trajectory.size()*poi_trajectory.size())*sizeof(double));
	if(!work)
		fprintf(stderr, "memory allocation request failed in main()\n");
	cout << poi_trajectory.size() << " " << measurement.size() << endl;
	int ret = 0;//dlevmar_dif(ObjectiveFunction_poi_trajectory, dPOI_trajectory, dMeasurement, poi_trajectory.size(), measurement.size(),
	...
	vector<double> &vx, vector<double> &vy, vector<double> &vz, double lambda)
{
	...
	for (int iSeg = 0; iSeg < startFrame.size(); iSeg++)
	{
		for (int iTime = startFrame[iSeg]; iTime < min(iTime+time_window, endFrame[iSeg]); iTime++)
		{
			...
			opt[4] = 0;
			double info[12];
			double *work = 0;// (double*)malloc((LM_DIF_WORKSZ(3*vX.size(), measurement.size())+3*vX.size()*3*vX.size())*sizeof(double));
			if(!work)
				fprintf(stderr, "memory allocation request failed in main()\n");
			//cout << 3*vX.size() << " " << measurement.size() << endl;
			int ret = 0;//dlevmar_dif(ObjectiveFunction_poi_trajectory1_weight, dPOI_trajectory, dMeasurement, 3*vX.size(), measurement.size(),
						//			1e+3, opt, info, work, NULL, &adata);
			//PrintSBAInfo(info, measurement.size());
			free(work);
				 ...
				 vector<double> &vx, vector<double> &vy, vector<double> &vz, double lambda)
{
	...
	if(!work)
		fprintf(stderr, "memory allocation request failed in main()\n");
	cout << poi_trajectory.size() << " " << measurement.size() << endl;
	int ret = 0;//dlevmar_dif(ObjectiveFunction_poi_trajectory1, dPOI_trajectory, dMeasurement, poi_trajectory.size(), measurement.size(),
				//			1e+3, opt, info, work, NULL, &adata);
	PrintSBAInfo(info, measurement.size());
	free(work);
...
void POIRefinement(POI_Matches &poi, vector<int> vFrame_PV, vector<vector<CvMat *> > vvP, vector<vector<CvMat *> > vvV, vector<vector<double> > vvBandwidth, double lambda)
{
	...
	vector<double> vx, vy, vz;
	int poi_idx=0;
	for (int iFrame = poi.vFrame[0]; iFrame < 500/*poi.vFrame[poi.vFrame.size()-1]*/; iFrame++)	
	{
		vector<int>::const_iterator it1 = find(vFrame_PV.begin(), vFrame_PV.end(), iFrame);
		int idx = (int) (it1 - vFrame_PV.begin());
		adata.vvP_poi.push_back(vvP[idx]);
								  ...
								  CvMat *X11, CvMat *X12, CvMat *X21, CvMat *X22, CvMat *pi)
{
	...
	double *work = 0;// (double*)malloc((LM_DIF_WORKSZ(XParameter.size(), feature2DParameter.size())+XParameter.size()*XParameter.size())*sizeof(double));
	if(!work)
		fprintf(stderr, "memory allocation request failed in main()\n");

	int ret = 0;//dlevmar_dif(Projection3Donto2D_Patch, dXParameter, dFeature2DParameter, XParameter.size(), feature2DParameter.size(),
				//		  1e+3, opt, info, work, NULL, &adata);
...
void TriangulationRefinement(CvMat *K, double omega, vector<CvMat *> vP, vector<double> vx, vector<double> vy, double &X, double &Y, double &Z)
{
	...
	int ret = 0;//0;//dlevmar_dif(Projection3Donto2D_STR_fast_SO, dXParameter, dFeature2DParameter, XParameter.size(), feature2DParameter.size(),
					//	  1e+3, opt, info, work, NULL, &adata);
...
void FaceRefinement(vector<CvMat *> &vX1, vector<CvMat *> vP1, vector<CvMat *> vx1, vector<int> vIdx1)
{
	...
	for (int iw = 0; iw < vX1.size(); iw+=stepsize)
	{
		...
		int ret = 0;//dlevmar_dif(Projection3Donto2D_FaceRefinement, dXParameter, dFeature2DParameter, XParameter.size(), feature2DParameter.size(),
					//		  1e+3, opt, info, work, NULL, &adata);
...
void TriangulationRefinement_NoDistortion(vector<vector<CvMat *> > vvP, vector<vector<double> > vvx, vector<vector<double> > vvy, vector<double> &vX, vector<double> &vY, vector<double> &vZ)
{
	...
	int ret = 0;//dlevmar_dif(Projection3Donto2D_STR_fast_SO_NoDistortion_All, dXParameter, dFeature2DParameter, XParameter.size(), feature2DParameter.size(),
				//		  1e+3, opt, info, work, NULL, &adata);
...
void TriangulationRefinement_NoDistortion(CvMat *K, vector<CvMat *> vP, vector<double> vx, vector<double> vy, double &X, double &Y, double &Z)
{
	...
	int ret = 0;//dlevmar_dif(Projection3Donto2D_STR_fast_SO_NoDistortion, dXParameter, dFeature2DParameter, XParameter.size(), feature2DParameter.size(),
				//		  1e+3, opt, info, work, NULL, &adata);
...
//		fprintf(stderr, "memory allocation request failed in main()\n");
//
//	int ret = 0;//dlevmar_dif(Projection3Donto2D_STR_Ray_ICCV, dXParameter, dFeature2DParameter, XParameter.size(), feature2DParameter.size(),
//						  1e+3, opt, info, work, NULL, &adata);
//
//	X = dXParameter[0];
//	Y = dXParameter[1];
...
void CfMRefinement(Camera &virtualCamera, vector<Camera> &vCamera, vector<CvMat *> &vT)
{
	...
	double info[12];
	cout << "Aa" << endl;
	double *work = 0;// (double*)malloc((LM_DIF_WORKSZ(cameraParameter.size(), feature2DParameter.size())+cameraParameter.size()*cameraParameter.size())*sizeof(double));
	if(!work)
		fprintf(stderr, "memory allocation request failed in main()\n");
	int ret = 0;//dlevmar_dif(Projection3Donto2D_CfM, dCameraParameter, dFeature2DParameter, cameraParameter.size(), feature2DParameter.size(),
				//		  1e+3, opt, info, work, NULL, &adata);
	cout << "done" << endl;
	for (int iFrame = 0; iFrame < virtualCamera.vTakenFrame.size(); iFrame++)
	{
...
void CfMRefinement_corner(vector<CvMat *> &vH0i, vector<Camera> vCamera, vector<double> &vReproj_x, vector<double> &vReproj_y)
{
	...
	if(!work)
		fprintf(stderr, "memory allocation request failed in main()\n");
	int ret = 0;//dlevmar_dif(Projection3Donto2D_CfM_corner, dCameraParameter, dFeature2DParameter, cameraParameter.size(), feature2DParameter.size(),
				//		  1e+3, opt, info, work, NULL, &adata);
	cout << "done" << endl;
...
void CfMRefinement_ECCM_Each(CvMat *H0i, vector<double> vx1, vector<double> vy1, vector<double> vx2, vector<double> vy2)
{
	...
	if(!work)
		fprintf(stderr, "memory allocation request failed in main()\n");
	int ret = 0;//dlevmar_dif(Projection3Donto2D_CfM_ECCM, dCameraParameter, dFeature2DParameter, cameraParameter.size(), feature2DParameter.size(),
				//		  1e+3, opt, info, work, NULL, &adata);
	cout << "done" << endl;
...
void CfMRefinement_ECCM_Each(CvMat *H0i, vector<double> vx1, vector<double> vy1, vector<double> vx2, vector<double> vy2, vector<double> vx3, vector<double> vy3)
{
	...
	if(!work)
		fprintf(stderr, "memory allocation request failed in main()\n");
	int ret = 0;//dlevmar_dif(Projection3Donto2D_CfM_ECCM1, dCameraParameter, dFeature2DParameter, cameraParameter.size(), feature2DParameter.size(),
				//		  1e+3, opt, info, work, NULL, &adata);
	cout << "done" << endl;
...
void CfMRefinement_ECCM_Final(vector<CvMat *> vH_inv_r, vector<vector<double> > vVx1, vector<vector<double> > vVy1, vector<vector<double> > vVx2, vector<vector<double> > vVy2)
{
	...
	if(!work)
		fprintf(stderr, "memory allocation request failed in main()\n");
	int ret = 0;//dlevmar_dif(Projection3Donto2D_CfM_ECCM_Final, dCameraParameter, dFeature2DParameter, cameraParameter.size(), feature2DParameter.size(),
				//		  1e+3, opt, info, work, NULL, &adata);
	cout << "done" << endl;
...
void AbsoluteCameraPoseRefinement(CvMat *X, CvMat *x, CvMat *P, CvMat *K)
{
	...
	int ret = 0;//dlevmar_dif(Projection3Donto2D_MOT_fast, dCameraParameter, dFeature2DParameter, cameraParameter.size(), feature2DParameter.size(),
				//		  1e+3, opt, info, work, NULL, &adata);
...
void SparseBundleAdjustment_TEMPORAL_LEVMAR(vector<Feature> vFeature, vector<Theta> &vTheta, vector<Camera> &vCamera)
{
	...
	//double *work = 0;// (double*)malloc((LM_DIF_WORKSZ(cameraParameter.size(), feature2DParameter.size()+nFeatures)+cameraParameter.size()*cameraParameter.size())*sizeof(double));
	//if(!work)
	//	fprintf(stderr, "memory allocation request failed in main()\n");

	//int ret = 0;//dlevmar_dif(ProjectionThetaonto2D_TEMPORAL_LEVMAR, dCameraParameter, dFeature2DParameter, cameraParameter.size(), feature2DParameter.size()+nFeatures,
	//	1e+3, opt, info, work, NULL, &adata);
	//cout << ret << endl;
	//PrintSBAInfo(info);
	//RetrieveParameterFromSBA_TEMPORAL(dCameraParameter, vCamera, vTheta);
...
int ExcludePointAtInfinity_mem_fast(CvMat *X, CvMat *P1, CvMat *P2, CvMat *K1, CvMat *K2, vector<int> &featureID, vector<int> &excludedFeatureID, vector<vector<double> > &cX)
{
	...
	for (int i = 0; i < X->rows; i++)
	{
		...
		double inner = v1x*v2x+v1y*v2y+v1z*v2z;
		vInner.push_back(inner);
		if ((abs(inner) < cos(PI/180*10)) && (inner > 0))
		{			
			vector<double> cX_vec;
...
void OrientationRefinement(CvMat *R_1, CvMat *R_F, vector<int> vFrame1, vector<int> vFrame2, vector<CvMat*> &vM, vector<CvMat*> &vm, vector<CvMat *> &vx1, vector<CvMat *> &vx2)
{
	...
	//double *work = 0;// (double*)malloc((LM_DIF_WORKSZ(cameraParameter.size(), measurement.size())+cameraParameter.size()*cameraParameter.size())*sizeof(double));
	//if(!work)
	//	fprintf(stderr, "memory allocation request failed in main()\n");

	//int ret = 0;//dlevmar_dif(ObjectiveOrientationRefinement, dCameraParameter, dmeasurement, cameraParameter.size(), measurement.size(),
									               ...
									               vector<int> vFrame1_r, vector<int> vFrame2_r, vector<CvMat*> &vM_r, vector<CvMat*> &vm_r, vector<CvMat *> &vx1_r, vector<CvMat *> &vx2_r)
{
	...
	//	int ret = 0;//dlevmar_dif(ObjectiveOrientationRefinement1, dCameraParameter, dmeasurement, cameraParameter.size(), measurement.size(),
	//		1e+2, opt, info, work, NULL, &adata);
